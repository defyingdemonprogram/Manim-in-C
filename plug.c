...
static float animation_value(Animation animation, Keyframe *kfs, size_t kfs_count) {
    assert(kfs_count > 0);

    Keyframe *kf = &kfs[animation.i%kfs_count];
    float t = animation.duration/kf->duration;
